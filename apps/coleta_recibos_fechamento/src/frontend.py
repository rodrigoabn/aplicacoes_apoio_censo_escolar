"""Frontend (Streamlit) da COLETA DE RECIBOS DE FECHAMENTO — MODO LEITURA.

Coleta: login, senha e upload da lista de escolas. Dispara a coleta (apenas
leitura no site) e, ao final, disponibiliza os recibos (PDF) de cada escola
compactados em um único arquivo ZIP para download. Nenhuma escrita é feita no
site Educacenso.
"""
...
def render_receipt_frontend(logo_base64=None):
    config = CONFIG
    ...
    with col_help:
        ...
        st.markdown("""
        - **Modo leitura:** a aplicação apenas navega e baixa os recibos do site; não altera dados no INEP.
        ...
        - Ao concluir, os recibos (PDF) de todas as escolas ficam disponíveis em um único **arquivo ZIP** para download.
        """, unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)
    ...
    if st.session_state[f"{p}finalized"]:
        if st.session_state[f"{p}zip"]:
            ...
            st.markdown(
                ...
                "Motivo: Nenhum recibo disponível para download.</span>",
                unsafe_allow_html=True,
            )
...
def _scrape_worker(config, login, senha, escolas, stop_event, lock, shared):
    ...
    from .receipt_scraper import run_receipt_scraping
    ...
    try:
        with sync_playwright() as pw:
            ...
            try:
                outputs, failures = run_receipt_scraping(
        ...
        for escola, data in outputs:
            try:
                zipper.append_pdf(escola["codigo_inep"], escola.get("nome_unidade", ""), data)
            except Exception as exc:  # noqa: BLE001
                with lock:
...
def _status_color(estado: str) -> str:
    ...
    if estado.startswith("erro") or estado.startswith("Falha no login") or estado in (
        "Recibo de fechamento indisponível.",
    ):
...
def _finalize(config: dict, count: int) -> None:
    ...
    try:
        ...
        if count > 0:
            ...
            if interrupted:
                st.success(
                    f"Coleta interrompida. {count} recibo(s) no arquivo "
                    f"'{nome_arquivo}'. A escola em andamento foi descartada."
                )
            else:
                st.success(
                    f"Concluído. {count} recibo(s) no arquivo '{nome_arquivo}', "
            ...
            # Nenhum PDF coletado: não gera ZIP; limpa temporários.
            zipper.reset_temp()
            ss[f"{p}zip"] = None
            st.markdown(
                "<span style='color:#ef4444'>Arquivo Zip não gerado. "
        ...
        if failures:
            ...
            st.markdown(
                ...
                "não tiveram recibo coletado.</span>",
                unsafe_allow_html=True,
            )
            st.download_button(
                label="Baixar escolas não coletadas (XLSX)",
