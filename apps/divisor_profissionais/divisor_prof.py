...
def load_csv(file):
    ...
    try:
        ...
        # A primeira linha é um super-cabeçalho ('Dados cadastrais'/'Vínculos');
        # descartada, usando a segunda linha como cabeçalho real da tabela.
        df = pd.read_csv(file, sep=delimiter, encoding=encoding_detected, header=1)
...
def _format_date_input():
    ...
    raw = st.session_state.get("divisor_prof_date_raw", "")
    ...
    if len(digits) >= 5:
        ...
        formatted = digits
    st.session_state["divisor_prof_date_raw"] = formatted
...
def generate_xlsx(df, school_name, collection_date):
    ...
    name_col  = find_column(df, ["Nome", "nome do profissional", "profissional", "docente"])
...
# ---------------------------------------------------------------------------
# Página do DIVISOR DE PROFISSIONAIS POR TURMAS
# ---------------------------------------------------------------------------

def show_divisor_prof_page(logo_base64):
    """Renderiza a interface completa do DIVISOR DE PROFISSIONAIS POR TURMAS."""
    ...
    if logo_base64:
        ...
        <div class="title-container" style="display:flex; align-items:center; gap:1.5rem;">
            ...
            <div>
                ...
                <p style="margin-top:0.3rem; color:#bfdbfe; font-size:1rem; margin-bottom:0;">
                    Organize o Relatório de Profissionais do Educacenso (CSV ou XLSX) em abas separadas por turma.
                </p>
            </div>
        </div>"""
    else:
        ...
        <div class="title-container">
            <h1>🏫 Relatórios de Profissionais Escolares (Organizar por Turma)</h1>
    ...
    if file_loaded and df is not None:
        with col_preview:
            ...
            if class_col and name_col:
                ...
                <table>
                    ...
                    <tbody>
                        <tr><td>Total de Profissionais</td><td style="text-align:right; font-weight:700; color:var(--text-primary);">{len(df)}</td></tr>
        ...
        with col_preview:
            ...
            2. **Envie** o Relatório de Profissionais baixado do Educacenso, em formato CSV ou XLSX.
            3. O sistema irá:
               - Agrupar profissionais por turma (**Nome da turma**).
